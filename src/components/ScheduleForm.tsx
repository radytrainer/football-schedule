...
const CATEGORIES: Category[] = ['Boys', 'Girls']

const SPORT_ICONS: Record<Sport, string> = { Volleyball: '🏐', Football: '⚽' }
const CATEGORY_ICONS: Record<Category, string> = { Boys: '👦', Girls: '👧' }
...
export default function ScheduleForm({ initial, onSubmit, onCancel }: Props) {
  ...
  const [category, setCategory] = useState<Category>(initial?.category ?? 'Boys')
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-5">
              ...
              <div className="flex gap-2">
                {SPORTS.map(s => (
                  <button
                    ...
                    type="button"
                    onClick={() => setSport(s)}
                    className={`flex-1 py-2 px-2 rounded-xl border-2 text-sm font-medium transition-all ${
                      sport === s
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={`w-full border rounded-xl px-3 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition-colors ${
                  errors.date ? 'border-rose-400 bg-rose-50' : 'border-gray-200 bg-white'
                }`}
              />
            ...
            <div>
              ...
              <input
                ...
                className={`w-full border rounded-xl px-3 py-2.5 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition-colors ${
                  errors.time ? 'border-rose-400 bg-rose-50' : 'border-gray-200 bg-white'
                }`}
              />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={`w-full border rounded-xl px-3 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition-colors ${
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent resize-none transition-colors"
            />
          </div>
          ...
          {(teamA || teamB) && (
            <div className="rounded-xl bg-indigo-50 border border-indigo-100 px-4 py-3">
              <p className="text-xs font-semibold text-indigo-400 uppercase tracking-wide mb-1.5">Preview</p>
          ...
          <div className="flex gap-3 pt-1">
            <button
              type="submit"
              className="flex-1 bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 text-white font-semibold py-2.5 rounded-xl transition-colors shadow-sm"
            >
            ...
            <button
              type="button"
              onClick={onCancel}
              className="flex-1 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 font-semibold py-2.5 rounded-xl transition-colors"
            >
              Cancel
            </button>
          </div>
