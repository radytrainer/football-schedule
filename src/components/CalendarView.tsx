...
export default function CalendarView({ matches, onAddNew }: Props) {
  if (matches.length === 0) {
    return (
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm py-20 text-center">
        <div className="text-5xl mb-4">📅</div>
        <p className="text-gray-700 font-semibold text-lg">No schedules to display</p>
        <p className="text-gray-400 text-sm mt-1 mb-5">Add matches or adjust your filter</p>
        <button
          onClick={onAddNew}
          ...
          <div key={date} className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className={`px-5 py-3 flex items-center gap-3 ${isToday ? 'bg-indigo-600' : 'bg-gray-700'}`}>
              <div className="flex-1 min-w-0">
                <p className="text-white font-semibold text-sm">{formatDayHeader(date)}</p>
              </div>
              {isToday && (
              ...
              <span className="shrink-0 text-xs bg-white/20 text-white font-medium px-2.5 py-0.5 rounded-full">
            ...
            <div className="divide-y divide-gray-100">
              {dayMatches.map(m => (
                <div
                  key={m.id}
                  className={`flex gap-4 px-5 py-4 border-l-4 hover:bg-gray-50 transition-colors ${SPORT_BORDER[m.sport]}`}
                >
                  {/* Time */}
                  <div className="w-20 shrink-0 text-right pt-0.5">
                    <p className="text-sm font-semibold text-gray-700">{formatTime(m.time)}</p>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      <span className={`px-2 py-0.5 rounded-lg text-xs font-semibold ${CATEGORY_PILL[m.category]}`}>
                        {m.category}
                      </span>
                    </div>
